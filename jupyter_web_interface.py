        ...
        with self.status_output:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    cmd = [
                        "bash", "-c",
                        f"source ~/.bashrc && conda activate hunyuan-i2v && python sample_image2video.py --image_path {image_path} --prompt '{self.prompt_text.value}' --video_length {self.video_length.value} --infer_steps {self.infer_steps.value} --i2v_resolution {self.resolution.value}"
                    ]
